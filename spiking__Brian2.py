...
def get_current_performance(performance, current_example_num):
    ...
    performance[current_evaluation] = correct / float(update_interval) * 100 #performance[current_evaluation] = correct / float(update_interval) * 100
    if (current_example_num + 1) == num_examples:
        performance[current_evaluation+1] = correct / float(update_interval) * 100
    return performance
...
if num_examples <= 24:
    ...
    update_interval = 2696
    weight_update_interval = 100
if num_examples <= 2696:
    save_connections_interval = 2696
else:
    save_connections_interval = 2696
    update_interval = 2696
...
outputNumbers = np.zeros((num_examples, 8))
if not test_mode:
...
while j < (int(num_examples)):
    ...
    if test_mode:
        if use_testing_set:
            spike_rates = testing['x'][j%24,:,:].reshape((n_input)) / 8. *  input_intensity
        else:
            spike_rates = training['x'][j%2696,:,:].reshape((n_input)) / 8. *  input_intensity
...
#     print('run number:', j+1, 'of', int(num_examples))
    ...
    if np.sum(current_spike_count) < 5:
        ...
        if test_mode and use_testing_set:
            input_numbers[j] = testing['y'][j%24][0]
        else:
            input_numbers[j] = training['y'][j%2696][0]
        outputNumbers[j,:] = get_recognized_number_ranking(assignments, result_monitor[j%update_interval,:])
        if j % 100 == 0 and j > 0:
            print('runs done:', j, 'of', int(num_examples))
        if j % update_interval == 0 and j > 0: #update_interval
            if do_plot_performance:
                unused, performance = update_performance_plot(performance_monitor, performance, j, fig_performance)
                print('Classification performance', performance[:int(j/update_interval+1)])
        if j % (num_examples-1) == 0 and j > 0: #update_interval
            if do_plot_performance:
                unused, performance = update_performance_plot(performance_monitor, performance, j, fig_performance)
                print('Classification performance', performance[:int(j/update_interval+2)])
        for i,name in enumerate(input_population_names):
            input_groups[name+'e'].rates = 0 * Hz
        if (j+1)%1000==0:
            #plt.figure(j+10)
