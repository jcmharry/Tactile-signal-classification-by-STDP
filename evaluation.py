...
def get_labeled_data(picklename, bTrain = True):
    ...
    if os.path.isfile('%s.pickle' % picklename):
        ...
        if bTrain:
            ...
            images = open(data_path + 't10k-images.idx3-ubyte','rb')
            labels = open(data_path + 't10k-labels.idx1-ubyte','rb')
...
test_results = np.zeros((8, end_time_testing-start_time_testing))
test_results_max = np.zeros((8, end_time_testing-start_time_testing))
test_results_top = np.zeros((8, end_time_testing-start_time_testing))
test_results_fixed = np.zeros((8, end_time_testing-start_time_testing))
...
print(assignments)
counter = 0 
num_tests = end_time_testing / 24
sum_accurracy = [0] * int(num_tests)
while (counter < num_tests):
    end_time = min(end_time_testing, 24*(counter+1))
    start_time = 24*counter
    test_results = np.zeros((8, end_time-start_time))
    ...
    for i in np.where(difference != 0)[0]:
        print(testing_input_numbers[i],test_results[0,i])
